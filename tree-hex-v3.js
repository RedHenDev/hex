...
AFRAME.registerComponent('tree-hex-manager', {
    ...
    createGeometries: function() {
      // Use the same hex prism geometry as terrain for trunk and foliage
      // Trunk hex prism geometry
      this.trunkGeometry = new THREE.HexagonGeometry(
        this.data.trunkBaseRadius, // size (radius)
        1.0,                       // height (unit, will be scaled per segment)
        false                      // flatTop (false for pointy-top, matches terrain)
      );

      // Foliage hex prism geometry
      this.foliageGeometry = new THREE.HexagonGeometry(
        1.0,                       // size (radius)
        this.data.foliageHeight,   // height
        false
      );

      // Branch triangle geometry connecting trunk to foliage (unchanged)
    ...
    createTreePool: function() {
      ...
      // Ensure visibility and correct rendering order
      this.trunkMesh.frustumCulled = false;
      this.foliageMesh.frustumCulled = false;
      if (this.data.enableBranches) {
        this.branchMesh.frustumCulled = false;
      }
      this.trunkMesh.renderOrder = 1;
      this.foliageMesh.renderOrder = 2;
      if (this.data.enableBranches) {
        this.branchMesh.renderOrder = 3;
      }
